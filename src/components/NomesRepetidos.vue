...
export default {
  ...
  computed: {
    nomesRepetidos() {
      ...
      return listaFiltrada.map((inscricao, indexInscricao) => {
        
        const semelantes = listaFiltrada.filter((i, index) => {
          return levenshtein(i.nome, inscricao.nome) < this.distancia && indexInscricao != index
        }).map(i => i.nome)
